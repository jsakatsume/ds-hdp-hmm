...
def sample_one_step_ahead(
    zt, yt, n_mat, ysum, beta_vec, beta_new, alpha0, gamma0, dir0, rho0, K
):
    T = len(zt)
    n_multi = sum(yt[0])
    m_multi = len(yt[0])
    ...
    for t in range(1, T):
        ...
        tmp_vec = np.arange(K)
        zt_dist = (alpha0 * beta_vec + n_mat[j] + rho0 * (j == tmp_vec)) / (
...
def init_gibbs(rho0, alpha0, gamma0, dir0, T, yt):
    ...
    zt, n_mat, ysum, beta_vec, beta_new, K = sample_one_step_ahead(
        zt, yt, n_mat, ysum, beta_vec, beta_new, alpha0, gamma0, dir0, rho0, K
    )
...
def init_gibbs_full_bayesian(
    alpha0_a_pri, alpha0_b_pri, gamma0_a_pri, gamma0_b_pri, c_pri, d_pri, dir0, T, yt
):
    K = 1
    zt = np.zeros(T, dtype="int")
...
def init_gibbs_full_bayesian_regular(
    alpha0_a_pri, alpha0_b_pri, gamma0_a_pri, gamma0_b_pri, dir0, T, yt
):
    K = 1
    zt = np.zeros(T, dtype="int")
    ...
    zt, n_mat, ysum, beta_vec, beta_new, K = sample_one_step_ahead(
        zt, yt, n_mat, ysum, beta_vec, beta_new, alpha0, gamma0, dir0, 0, K
    )
...
def sample_zw(zt, yt, n_mat, ysum, beta_vec, beta_new, alpha0, gamma0, dir0, rho0, K):
    ...
    for t in range(1, T - 1):
        ...
        post_cases = np.hstack(
            (zt_dist * ztplus1_dist * yt_dist, knew_dist * yt_knew_dist)
        )
        post_cases = post_cases / (post_cases.sum())
    ...
    zt, n_mat, ysum, beta_vec, beta_new, K = sample_last(
        zt, yt, n_mat, ysum, beta_vec, beta_new, alpha0, gamma0, dir0, rho0, K
    )
...
def sample_w(K, m_mat, beta_vec, alpha0, rho0):
    ...
    for j in range(K):
        w_vec[j] = binomial(
            m_mat[j, j], stick_ratio / (stick_ratio + beta_vec[j] * (1 - stick_ratio))
        )
...
def sample_gamma(K, m_mat_bar, gamma0, gamma0_a_pri, gamma0_b_pri):
    ...
    pi_m = (gamma0_a_pri + K - 1) / (
        gamma0_a_pri + K - 1 + m_mat_bar.sum() * (gamma0_b_pri - np.log(eta + eps))
    )
