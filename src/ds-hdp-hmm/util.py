...
def compute_log_marginal_lik_gaussian(
    K, yt, zt, prob_mat, mu0, sigma0, sigma0_pri, ysum, ycnt
):
    ...
    varn = np.hstack(
        (np.sqrt((sigma0**2) + varn), np.sqrt((sigma0**2) + (sigma0_pri**2)))
    )
    ...
    for t in range(T):
        if t == 0 and zt == -1:
            ...
            for j in range(K + 1):
                a_mat[t + 1, j] = sum(a_mat[t, :] * prob_mat[:, j]) * ss.norm.pdf(
                    yt[t], mun[j], varn[j]
                )
...
def compute_log_marginal_lik_multinomial(K, yt, zt, prob_mat, dir0, ysum):
    ...
    ## compute mu sigma posterior
    yt_dist = (
        ssp.loggamma(dir0_sum + ysum.sum(axis=1))
        - ssp.loggamma(dir0_sum + ysum.sum(axis=1) + n_multi)
    ) - np.sum(ssp.loggamma(dir0 + ysum), axis=1)
    yt_knew_dist = (
        ssp.loggamma(dir0_sum)
        - ssp.loggamma(dir0_sum + n_multi)
        - np.sum(ssp.loggamma(dir0))
    )
    ...
    for t in range(T):
        if t == 0 and zt == -1:
            ...
            a_mat[t + 1, j] = np.exp(
                ...
                + np.real(
                    np.sum(
                        ssp.loggamma(single_term[j] + yt[t]) - ssp.loggamma(1 + yt[t])
                    )
                )
            )
        else:
            for j in range(K + 1):
                a_mat[t + 1, j] = sum(a_mat[t, :] * prob_mat[:, j]) * np.exp(
                    ...
                    + np.real(
                        np.sum(
                            ssp.loggamma(single_term[j] + yt[t])
                            - ssp.loggamma(1 + yt[t])
                        )
                    )
                )
...
## compute log marginal likelihood p(y|pi,alpha,beta,sigma,z,w,kappa,yold)
def compute_log_marginal_lik_poisson(
    K, yt, zt, prob_mat, lam_a_pri, lam_b_pri, ysum, ycnt
):
    T = len(yt)
    m_multi = len(yt[0])
    a_mat = np.zeros((T + 1, K + 1))
        ...
        if t == 0 and zt == -1:
            ...
            a_mat[t + 1, j] = np.exp(
                np.sum(
                    np.log(
                        ss.nbinom.pmf(
                            yt[t], lam_a_post[j], lam_b_post[j] / (lam_b_post[j] + 1)
                        )
                    )
                )
            )
            ...
            for j in range(K + 1):
                yt_dist = np.sum(
                    np.log(
                        ss.nbinom.pmf(yt[t], lam_a_post, lam_b_post / (lam_b_post + 1))
                    ),
                    axis=1,
                )
                yt_knew_dist = np.sum(
                    np.log(ss.nbinom.pmf(yt[t], lam_a_pri, lam_b_pri / (lam_b_pri + 1)))
                )
...
def compute_log_marginal_lik_ar(
    K,
    yt,
    zt,
    prob_mat,
    ...
    for t in range(1, T):
        ...
        mun = np.array(
            [np.matmul(s_y_ybar[ik], tmp[ik]) for ik in range(K)]
        )  # K by m_multi mat
        yt_dist = np.array(
            [
                multivariate_t_distribution(
                    yt[t],
                    mun[ik],
                    s_y_cond_ybar_plus_s0[ik] * (1 + (1 / nun[ik])) / dff[ik],
                    dff[ik],
                )
                for ik in range(K)
            ]
        ...
        yt_knew_dist = multivariate_t_distribution(
            yt[t],
            mu_new,
            S0 * (1 + (1 / nu_new)) / (n0 + 1 - m_multi),
            (n0 + 1 - m_multi),
        )
...
def compute_log_marginal_lik_ar_approx(L, yt, zt, prob_mat, suff_stats):
    ...
    for t in range(1, T):
        ...
        nun = 1 / np.matmul(tmp, yt[t - 1])  # length K vector
        mun = np.array(
            [np.matmul(suff_stats["s_y_ybar"][ik], tmp[ik]) for ik in range(L)]
        )  # K by m_multi mat
        ...
        yt_dist = np.array(
            [
                multivariate_t_distribution(
                    yt[t],
                    mun[ik],
                    suff_stats["s_y_cond_ybar_plus_s0"][ik]
                    * (1 + (1 / nun[ik]))
                    / dff[ik],
                    dff[ik],
                )
...
def compute_log_marginal_lik_ar_approx_parallel(
    L, prob_mat, pi_init, suff_stats, yt_ls
):
    ## if zt is -1, then yt is a brand new sequence starting with state 0
    ## if zt is not -1, then it's the state of time point before the first time point of yt
    log_marginal_lik_ls = []
    ...
    for yt in yt_ls:
        ...
        for t in range(1, T):
            ## compute y marginal likelihood
            tmp = np.matmul(
                suff_stats["s_ybar_ybar_inv"], yt[t - 1]
            )  # K by m_multi mat
            nun = 1 / np.matmul(tmp, yt[t - 1])  # length K vector
            mun = np.array(
                [np.matmul(suff_stats["s_y_ybar"][ik], tmp[ik]) for ik in range(L)]
            ...
            if t == 1:
                ...
                a_mat[t] = (
                    np.matmul(a_mat[t - 1].reshape(1, -1), prob_mat).reshape(-1)
                    * yt_dist
                )
            c_vec[t - 1] = sum(a_mat[t])
            a_mat[t] /= c_vec[t - 1]
...
def compute_log_marginal_lik_ar_fmp(
    L, prob_mat, pi_init, suff_stats, yt_all_ls, n_cores
):
    chunks = np.array_split(yt_all_ls, n_cores)
    # chunks = [yt_all_ls[i::n_cores] for i in range(n_cores)];
    func = partial(
        compute_log_marginal_lik_ar_approx_parallel, L, prob_mat, pi_init, suff_stats
    )
...
def compute_log_marginal_lik_poisson_approx_parallel(
    L, prob_mat, pi_init, suff_stats, yt_ls
):
    ## if zt is -1, then yt is a brand new sequence starting with state 0
    ## if zt is not -1, then it's the state of time point before the first time point of yt
    ...
    for yt in yt_ls:
        ...
        for t in range(T):
            ## compute y marginal likelihood
            yt_dist = np.sum(
                np.log(ss.nbinom.pmf(yt[t], lam_a_post, lam_b_post / (lam_b_post + 1))),
                axis=1,
            )
            yt_dist = np.exp(yt_dist)
...
def compute_log_marginal_lik_poisson_fmp(
    L, prob_mat, pi_init, suff_stats, yt_all_ls, n_cores
):
    chunks = np.array_split(yt_all_ls, n_cores)
    # chunks = [yt_all_ls[i::n_cores] for i in range(n_cores)];
    func = partial(
        compute_log_marginal_lik_poisson_approx_parallel,
        L,
        prob_mat,
        pi_init,
        suff_stats,
...
def compute_log_obs_lik_ar_fmp(L, prob_mat, pi_init, lik_params, yt_all_ls, n_cores):
    ...
    # chunks = [yt_all_ls[i::n_cores] for i in range(n_cores)];
    func = partial(
        compute_log_obs_lik_ar_approx_parallel, L, prob_mat, pi_init, lik_params
    )
