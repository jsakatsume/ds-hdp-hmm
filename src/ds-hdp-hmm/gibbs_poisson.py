...
def init_gibbs(rho0, rho1, alpha0, gamma0, lam_a_pri, lam_b_pri, T, yt):
    ...
    zt, wt, n_mat, ysum, ycnt, beta_vec, kappa_vec, beta_new, kappa_new, K = (
...
def init_gibbs_full_bayesian(
    ...
    gamma0_b_pri,
    lam_a_pri,
    lam_b_hyper_pri_shape,
    lam_b_hyper_pri_rate,
    T,
    yt,
):
    K = 1
    ...
    n_mat = np.array(
        [[0]]
    )  # t = 0 count as wt=0, don't need to infer wt, t=T will add one
    ysum = np.array([yt[0]])
    ycnt = np.array([np.ones(len(yt[0]))])

    lam_b_pri = gamma(
        lam_b_hyper_pri_shape, 1 / (lam_b_hyper_pri_rate), size=len(yt[0])
    )
...
def sample_last(
    ...
    lam_b_post = lam_b_pri + ycnt
    yt_dist = np.sum(
        np.log(ss.nbinom.pmf(yt[t], lam_a_post, lam_b_post / (lam_b_post + 1))), axis=1
    )
    yt_dist = np.exp(yt_dist)
    yt_knew_dist = np.sum(
        np.log(ss.nbinom.pmf(yt[t], lam_a_pri, lam_b_pri / (lam_b_pri + 1)))
...
def sample_zw(
    ...
    for t in range(1, T - 1):
        ...
        zt_dist = (alpha0 * beta_vec + n_mat[j]) / (alpha0 + n_mat[j].sum())
        ztplus1_dist = (
            alpha0 * beta_vec[l] + n_mat[:, l] + (j == l) * (j == tmp_vec)
        ) / (alpha0 + n_mat.sum(axis=1) + (j == tmp_vec))
        knew_dist = (
            (alpha0**2) * beta_vec[l] * beta_new / (alpha0 * (alpha0 + n_mat[j].sum()))
        )
        ...
        post_cases = np.hstack(
            (
                post_cases,
                (1 - kappa_vec[j]) * (1 - kappa_vec) * zt_dist * ztplus1_dist * yt_dist,
                (1 - kappa_vec[j]) * (1 - kappa_new) * knew_dist * yt_knew_dist,
            )
        )
...
def sample_gamma(K, m_mat, gamma0, gamma0_a_pri, gamma0_b_pri):
    ...
    pi_m = (gamma0_a_pri + K - 1) / (
        gamma0_a_pri + K - 1 + m_mat.sum() * (gamma0_b_pri - np.log(eta + eps))
    )
...
def compute_rho_posterior(rho0, rho1, K, num_1_vec, num_0_vec):
    log_posterior = (
        K * (ssp.loggamma(rho0 + rho1) - ssp.loggamma(rho0) - ssp.loggamma(rho1))
...
def sample_rho(
    v0_range, v1_range, v0_num_grid, v1_num_grid, K, num_1_vec, num_0_vec, p
):
    ...
    for ii, v0 in enumerate(v0_grid):
        for jj, v1 in enumerate(v1_grid):
            ...
            posterior_grid[ii, jj] = compute_rho_posterior(
                rho0, rho1, K, num_1_vec, num_0_vec
            )
...
def sample_lam_b_pri(
    lam_b_hyper_pri_shape, lam_b_hyper_pri_rate, lam_a_pri, lam_mat, K
):
    lam_b_pri = gamma(
        lam_b_hyper_pri_shape + K * lam_a_pri,
