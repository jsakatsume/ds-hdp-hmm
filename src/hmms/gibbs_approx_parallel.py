...
from numpy.random import beta, binomial, dirichlet, gamma, multinomial, uniform
...
def init_suff_stats_func(prior_params, L, mode):
    if mode == "poisson":
        ...
        if "lam_b_pri" not in prior_params.keys():
            prior_params["lam_b_pri"] = gamma(
                prior_params["lam_b_hyper_pri_shape"],
                1 / (prior_params["lam_b_hyper_pri_rate"]),
                size=prior_params["m_multi"],
            )

        suff_stats["ysum"] = np.tile(
            prior_params["lam_a_pri"], [L, prior_params["m_multi"]]
        )
    ...
    if mode == "ar":
        suff_stats = {
            ...
            "s_y_y_plus_s0": [],
            "s_y_ybar": [],
            "s_y_cond_ybar_plus_s0": [],
            "dff": [],
            "V0_inv": [],
        }
        V0_inv = np.linalg.inv(prior_params["V0"])
        suff_stats["s_ybar_ybar_inv"] = np.tile(prior_params["V0"], [L, 1, 1])
        suff_stats["s_y_y_plus_s0"] = np.tile(
            prior_params["S0"]
            + np.matmul(np.matmul(prior_params["M0"], V0_inv), prior_params["M0"].T),
            [L, 1, 1],
        )
        suff_stats["s_y_ybar"] = np.tile(
            np.matmul(prior_params["M0"], V0_inv), [L, 1, 1]
        )
...
def update_suff_stats_base(yt, zt, L, suff_stats_base, mode):
    ...
    if mode == "ar":
        if not bool(suff_stats_base):
            suff_stats_base = {
                "ybar_ybar": np.zeros((L, m_multi, m_multi)),
                "y_y": np.zeros((L, m_multi, m_multi)),
                "y_ybar": np.zeros((L, m_multi, m_multi)),
                "ycnt": np.zeros(L),
            }
        for ii in range(L):
...
def sample_lik_params(suff_stats, mode):
    ...
    if mode == "ar":
        ...
        sigma_mat_post = np.array(
            [
                ss.invwishart.rvs(
                    df=suff_stats["dff"][ik],
                    scale=suff_stats["s_y_cond_ybar_plus_s0"][ik],
                )
                for ik in range(L)
            ]
        )  # L-d-d mat
        ...
        mun = np.matmul(suff_stats["s_y_ybar"], suff_stats["s_ybar_ybar_inv"]).reshape(
            L, -1, order="F"
        )  # L-d-d mat
        for ik in range(L):
            ...
            sample = (
                np.matmul(np.linalg.cholesky(covn), np.random.randn(m_multi * m_multi))
                + mun[ik]
            )
            a_mat_post[ik] = sample.reshape(
                m_multi, m_multi, order="F"
            )  ## sample dxd length vector
...
def compute_lik(yt, lik_params, mode):
    if mode == "poisson":
        lik_mat = np.sum(
            ss.poisson.logpmf(yt, np.expand_dims(lik_params["lam_post"], axis=1)),
            axis=-1,
        )
    ...
    if mode == "ar":
        ...
        for ik in range(L):
            for it in range(T - 1):
                lik_mat[it + 1, ik] = ss.multivariate_normal.pdf(
                    yt[it + 1], mean=mun[ik, it], cov=lik_params["sigma_mat_post"][ik]
                )
    return lik_mat
...
def init_suff_stats_base(L, mode, yt_ls):
    ...
    suff_stats_base = {}
    for yt in yt_ls:
        suff_stats_base = update_suff_stats_base(
            yt[:, :-1], yt[:, -1], L, suff_stats_base, mode
        )
...
def update_suff_stats(init_suff_stats, suff_stats_base, mode, L):
    ...
    if mode == "ar":
        suff_stats = copy.deepcopy(init_suff_stats)
        for ii in range(L):
            suff_stats["s_ybar_ybar_inv"][ii] = np.linalg.inv(
                suff_stats_base["ybar_ybar"][ii] + init_suff_stats["V0_inv"]
            )
            suff_stats["s_y_y_plus_s0"][ii] += suff_stats_base["y_y"][ii]
            suff_stats["s_y_ybar"][ii] += suff_stats_base["y_ybar"][ii]
            suff_stats["s_y_cond_ybar_plus_s0"][ii] = suff_stats["s_y_y_plus_s0"][
                ii
            ] - np.matmul(
                np.matmul(
                    suff_stats["s_y_ybar"][ii], suff_stats["s_ybar_ybar_inv"][ii]
                ),
                suff_stats["s_y_ybar"][ii].T,
            )
            suff_stats["dff"][ii] += suff_stats_base["ycnt"][ii]
    return suff_stats


def init_gibbs_from_input_fmp(
    p,
    v0_range,
    v1_range,
    alpha0_a_pri,
...
def sample_zw(pi_bar, kappa_vec, pi_init, L, lik_params, mode, yt_ls):
    ...
    for yt in yt_ls:
        ...
        for it in range(T - 2, -1, -1):
            message_mat[it] = ((message_mat[it + 1] * lik_mat[it + 1]) * pi_all).sum(
                axis=-1
            )
        ...
        for t in iterator:
            ...
            post_cases = np.hstack(
                (kappa_vec[j] * prob_vec[j], pi_bar[j] * (1 - kappa_vec[j]) * prob_vec)
            )
            post_cases /= post_cases.sum()
            # print(post_cases);
    ...
    return {
        ...
        "num_1_vec": num_1_vec,
        "num_0_vec": num_0_vec,
        "suff_stats_base": suff_stats_base,
        "uniq": uniq,
        "zt": zt_all,
        "wt": wt_all,
    }
...
def sample_zw_fmp(
    yt_all_ls, pi_bar, kappa_vec, pi_init, L, lik_params, mode, init_suff_stats, n_cores
):
    ...
    zt, wt, n_mat, num_1_vec, num_0_vec, n_ft, K, uniq, suff_stats = update_all_stats(
        results, init_suff_stats, mode, L
    )
    return zt, wt, n_mat, num_1_vec, num_0_vec, n_ft, K, uniq, suff_stats
        ...
        for k in range(L):
            if n_mat[j, k] == 0:
                ...
                x_vec = binomial(
                    ...
                    alpha0
                    * beta_vec[k]
                    / (np.arange(n_mat[j, k]) + alpha0 * beta_vec[k]),
                )
    ...
    for j in range(L):
        if n_ft[j] == 0:
            ...
            x_vec = binomial(
                ...
                alpha0_init
                * beta_vec[j]
                / (np.arange(n_ft[j]) + alpha0_init * beta_vec[j]),
            )
            x_vec = np.array(x_vec).reshape(-1)
            m_init[j] = sum(x_vec)
...
def sample_pi(
    n_mat, n_ft, alpha0, alpha0_init, beta_vec
):  ## first time point won't affect pi_bar
    L = n_mat.shape[0]
    pi_bar = np.zeros((L, L))
    for k in range(L):
...
def sample_alpha(
    m_mat, n_mat, alpha0, m_init, n_ft, alpha0_init, alpha0_a_pri, alpha0_b_pri
):  ## first time point will definitely add one more table no matter what alpha is, so it won't affect alpha posterior
    r_vec = []
    ...
    alpha0 = gamma(
        alpha0_a_pri + (m_mat.sum()) - sum(s_vec),
        1 / (alpha0_b_pri - sum(np.log(r_vec + eps))),
    )  ## not consider first time point

    ## sample alpha_init
    nper = n_ft.sum()
    eta = beta(alpha0_init + 1, nper)
    ntab = m_init.sum()
    pi_m = (alpha0_a_pri + ntab - 1) / (
        alpha0_a_pri + ntab - 1 + nper * (alpha0_b_pri - np.log(eta + eps))
    )
    ...
    if indicator:
        ...
        alpha0_init = gamma(
            alpha0_a_pri + ntab - 1, 1 / (alpha0_b_pri - np.log(eta + eps))
        )
...
def sample_gamma(
    K, m_mat, m_init, gamma0, gamma0_a_pri, gamma0_b_pri
):  ## first time point will affect gamma
    num_tabs = m_mat.sum() + m_init.sum()
    eta = beta(gamma0 + 1, num_tabs)

    pi_m = (gamma0_a_pri + K - 1) / (
        gamma0_a_pri + K - 1 + num_tabs * (gamma0_b_pri - np.log(eta + eps))
    )
    indicator = binomial(1, pi_m)
...
def compute_rho_posterior(rho0, rho1, num_1_vec, num_0_vec):
    L = len(num_1_vec)
    log_posterior = (
        L * (ssp.loggamma(rho0 + rho1) - ssp.loggamma(rho0) - ssp.loggamma(rho1))
        + sum(ssp.loggamma(rho0 + num_1_vec))
        + sum(ssp.loggamma(rho1 + num_0_vec))
        - sum(ssp.loggamma(rho0 + rho1 + num_1_vec + num_0_vec))
    )
...
def sample_rho(v0_range, v1_range, v0_num_grid, v1_num_grid, num_1_vec, num_0_vec, p):
    ...
    for ii, v0 in enumerate(v0_grid):
        for jj, v1 in enumerate(v1_grid):
            rho0, rho1 = transform_var_poly(v0, v1, p)
            posterior_grid[ii, jj] = compute_rho_posterior(
                rho0, rho1, num_1_vec, num_0_vec
            )
...
def sample_lam_b_pri(lik_params, prior_params, uniq, K):
    prior_params["lam_b_pri"] = gamma(
        prior_params["lam_b_hyper_pri_shape"] + K * prior_params["lam_a_pri"],
        1
        / (
            prior_params["lam_b_hyper_pri_rate"]
            + (lik_params["lam_post"][np.array(uniq, dtype="int")]).sum(axis=0)
        ),
    )
    return prior_params
