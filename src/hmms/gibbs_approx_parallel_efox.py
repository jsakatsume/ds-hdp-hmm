...
def sample_zw(pi_bar, pi_init, L, lik_params, mode, yt_ls):
    ...
    for yt in yt_ls:
        ...
        for it in range(T - 2, -1, -1):
            message_mat[it] = ((message_mat[it + 1] * lik_mat[it + 1]) * pi_bar).sum(
                axis=-1
            )
...
def sample_zw_fmp(
    yt_all_ls, pi_bar, pi_init, L, lik_params, mode, init_suff_stats, n_cores
):
    ...
    zt, n_mat, n_ft, K, uniq, suff_stats = update_all_stats(
...
def sample_pi(
    n_mat, n_ft, alpha0, alpha0_init, rho0, beta_vec
...
def sample_concentration(
    m_mat, n_mat, alpha0, rho0, m_init, n_ft, alpha0_init, alpha0_a_pri, alpha0_b_pri
):
    r_vec = []
...
def sample_gamma(
    K, m_mat_bar, m_init, gamma0, gamma0_a_pri, gamma0_b_pri
