...
import numpy as np
from numpy.random import beta, binomial, multinomial
    ...
    for t in range(1, T):
        ...
        if not new_tab:
            ...
            if dish == len(m_mat):  # new dish!
                ...
                kappa.append(
                    beta(rho0, rho1)
                )  # next customer in a new restaurant, assign a new kappa
...
def sample_same_trans(K_real, p_real1, p_real2, p_real3, p_real4, T):
    kappa_real = np.hstack(
        (np.ones(int(K_real // 2)) * p_real1, np.ones(int(K_real // 2)) * p_real2)
    )
    ...
    trans_vec = np.hstack(
        (
            np.ones(int(K_real // 2)) * p_real4 / int(K_real // 2),
            np.ones(int(K_real // 2)) * p_real3 / int(K_real // 2),
        )
    )
