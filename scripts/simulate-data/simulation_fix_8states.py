...
zt_real, wt_real, kappa_real, trans_vec = sample_same_trans(
    8, 0.65, 0.9, 0.18, 0.8, 1000
)
dic_real = np.diag(np.ones(8)) * 0.88 + (0.12 / 8)
dic_real = dic_real / dic_real.sum(axis=1, keepdims=True)
yt_real = np.array(
    [multinomial(1, dic_real[zt_real][ii]) for ii in range(len(zt_real))]
)
trans_mat = trans_vec * np.expand_dims(1 - kappa_real, axis=-1) + np.diag(
    kappa_real
)  ## compute the real transtion matrix

np.savez(
    "../../data/fix_8states_multinomial_same_trans_diff_stick.npz",
    zt=zt_real,
    wt=wt_real,
    kappa=kappa_real,
    yt=yt_real,
...
np.savez(
    "../../data/test_fix_8states_multinomial_same_trans_diff_stick.npz",
    zt=zt_real,
    wt=wt_real,
    yt=yt_real,
)
...
zt_real, wt_real, kappa_real, trans_mat = sample_same_stick(
    K_real=8, p_real=0.8, T=1000, part=2
)
...
np.savez(
    "../../data/fix_8states_multinomial_diff_trans_same_stick.npz",
    zt=zt_real,
    wt=wt_real,
    kappa=kappa_real,
    yt=yt_real,
...
np.savez(
    "../../data/test_fix_8states_multinomial_diff_trans_same_stick.npz",
    zt=zt_real,
    wt=wt_real,
    yt=yt_real,
)
