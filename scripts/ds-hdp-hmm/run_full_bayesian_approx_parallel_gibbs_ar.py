...
n_cores = int(
    sys.argv[3]
)  ## number of cores for parallel. if not parallel, just set n_cores=1, but note that the yt_all_ls, and yt_test_ls should still be 3d array with the first dimension as 1
init_way = sys.argv[
    4
]  ## initialization way: from 'prior' or from 'hmm' (parametric hmm result)
file_name = sys.argv[5]  ## file name of data
rlt_path = sys.argv[6]  ## path to save results
L = int(
    sys.argv[7]
)  ## in general, set it to be twice the number of states is already good enough

dat = np.load("../../data/" + file_name + ".npz", allow_pickle=True)
yt_all_ls = dat[
    "yt_train"
]  ## 3d numpy array: nTrials x nTime-points-per-trial x ndim. note that nTime-points-per-trial can be different for each trial, but ndim has to be the same
yt_test_ls = dat["yt_test"]
del dat

m_multi = yt_all_ls[0].shape[-1]
prior_params = {
    "M0": np.zeros((m_multi, m_multi)),
    "V0": np.identity(m_multi),
    "S0": np.cov(np.concatenate(yt_all_ls, axis=0).T) * 0.75,
    "n0": m_multi + 2,
}  ## mniw prior parameters for ar observations, can adjust it based on data
...
for it in range(iters):
    zt, wt, n_mat, num_1_vec, num_0_vec, n_ft, K, uniq, suff_stats = sample_zw_fmp(
        yt_all_ls,
        pi_bar,
        kappa_vec,
        pi_init,
    ...
    ## sample hyperparams
    alpha0, alpha0_init = sample_alpha(
    ...
    rho0, rho1, posterior_grid = sample_rho(
        v0_range, v1_range, v0_num_grid, v1_num_grid, num_1_vec, num_0_vec, p
    )

    ## compute loglik
    if it % 10 == 0:
        pi_all = np.diag(kappa_vec) + np.matmul(np.diag(1 - kappa_vec), pi_bar)
        loglik_test = compute_log_marginal_lik_ar_fmp(
            L, pi_all, pi_init, suff_stats, yt_test_ls, n_cores
        )
    ...
    if it % 100 == 0:
        np.savez(
            rlt_path + file_name + "_full_bayesian_approx_rlt_" + str(seed) + ".npz",
            zt=zt_sample,
            wt=wt_sample,
            kappa=kappa_vec_sample,
            hyper=hyperparam_sample,
            loglik=loglik_test_sample,
            post=post_sample,
            pi_mat=pi_mat_sample,
            pi_init=pi_init_sample,
            uniq=uniq_sample,
            lik_params=lik_params_sample,
        )
