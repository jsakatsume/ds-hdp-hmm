...
dir0 = int(
    sys.argv[3]
)  ## dirichlet prior parameter for the parameter in multinomial observation
...
for it in range(iters):
    if it == 0:
        ...
        zt, wt, n_mat, ysum, beta_vec, kappa_vec, beta_new, kappa_new, K = sample_zw(
            zt,
            wt,
            yt_real,
            n_mat,
    ...
    if it % 10 == 0:
        ...
        _, loglik_test = compute_log_marginal_lik_multinomial(
            K, yt_test, -1, pi_mat, dir0, ysum
        )
...
for ii in range(len(zt_sample)):
    ...
    kappa_tmp = dict(
        (dic[jj], kappa_vec_sample[ii][jj])
        for jj in range(len(np.unique(zt_sample[ii])))
    )
...
np.savez(
    ...
    hamming=mismatch_vec,
    zt_permute=zt_sample_permute,
    kappa_permute=kappa_sample_permute,
