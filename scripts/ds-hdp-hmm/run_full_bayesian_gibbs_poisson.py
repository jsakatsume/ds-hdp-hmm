...
for it in range(iters):
    ...
    lam_b_pri = sample_lam_b_pri(
        lam_b_hyper_pri_shape, lam_b_hyper_pri_rate, lam_a_pri, lam_mat, K
    )

    ## compute loglik
    if it % 10 == 0:
        pi_mat = sample_pi_our(
            K, alpha0, beta_vec, beta_new, n_mat, kappa_vec, kappa_new
        )
        _, loglik_test = compute_log_marginal_lik_poisson(
            K, yt_test, zt[-1], pi_mat, lam_a_pri, lam_b_pri, ysum, ycnt
        )
        ...
        hyperparam_sample.append(
            np.hstack((np.array([alpha0, gamma0, rho0, rho1]), lam_b_pri))
        )

    if it % 100 == 0:
        np.savez(
            rlt_path + file_name + "_full_bayesian_rlt_" + str(seed) + ".npz",
            zt=zt_sample,
            wt=wt_sample,
