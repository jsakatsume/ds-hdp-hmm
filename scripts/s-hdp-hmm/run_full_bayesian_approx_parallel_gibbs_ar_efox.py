...
for it in range(iters):
    zt, n_mat, n_ft, K, uniq, suff_stats = sample_zw_fmp(
        yt_all_ls, pi_bar, pi_init, L, lik_params, mode, init_suff_stats, n_cores
    )
    ## sample hdp prior parameters
    m_mat, m_init, w_vec, m_mat_bar = sample_m_w_mbar(
        n_mat, n_ft, beta_vec, alpha0, alpha0_init, rho0
    )
    ...
    ## sample hyperparams
    concentration, alpha0_init = sample_concentration(
        m_mat,
        n_mat,
        alpha0,
        rho0,
    ...
    if it % 10 == 0:
        loglik_test = compute_log_marginal_lik_ar_fmp(
            L, pi_bar, pi_init, suff_stats, yt_test_ls, n_cores
        )
